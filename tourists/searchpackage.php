...
if (empty($search_results)) {
    // Fetch the category_id of the most relevant matching package
    $category_stmt = $dbh->prepare("SELECT category_id FROM Packages WHERE name LIKE :search OR location LIKE :search LIMIT 1");
    $category_stmt->bindValue(':search', '%' . $search_query . '%', PDO::PARAM_STR);
    $category_stmt->execute();
    $category = $category_stmt->fetch(PDO::FETCH_ASSOC);

    if ($category) {
        // Fetch related packages using the fetched category_id
        ...
        $related_stmt->bindValue(':category_id', $category['category_id'], PDO::PARAM_INT);
                ...
                <div class="col-md-4 mb-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text"><small class="text-muted">Rs <?php echo htmlspecialchars($package['price']); ?> | <?php echo htmlspecialchars($package['duration']); ?></small></p>
